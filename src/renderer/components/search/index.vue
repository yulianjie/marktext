    ...
    <div class="right-controls">
      <section class="search">
        ...
        >
          <input
            ...
            @keyup="search($event)"
            ref="search"
            :placeholder="$t('findInFile.searchPlaceholder')"
          >
          <div class="controls">
            <span class="search-result">{{ $t('findInFile.counter', { index: highlightIndex + 1, total: highlightCount }) }}</span>
            <span
      ...
      <section class="replace" v-if="type === 'replace'">
        <div class="input-wrapper replace-input">
          <input type="text" v-model="replaceValue" :placeholder="$t('findInFile.replacePlaceholder')">
        </div>
        <div class="button-group">
          <el-tooltip class="item"
            effect="dark"
            :content="$t('findInFile.replaceAll')"
            placement="top"
            :visible-arrow="false"
            :open-delay="1000"
          >
          ...
          <el-tooltip class="item"
            effect="dark"
            :content="$t('findInFile.replaceSingle')"
            placement="top"
            :visible-arrow="false"
            :open-delay="1000"
          >
    ...
    search (event) {
      ...
      if (isRegexp) {
        ...
        try {
          ...
          this.searchErrorMsg = this.$t('findInFile.invalidRegex', { query: searchValue })
          return
        }
        ...
        try {
          ...
          this.searchErrorMsg = this.$t('findInFile.regexMatchEmpty', { query: searchValue })
          return
        }
      }
